...
local taskmaster_killfaraway_range = CreateConVar("ttt_taskmaster_killfaraway_range", "25", FCVAR_REPLICATED, "The minimum distance (in meters) away a player can be to count for the 'Kill Faraway Player' task", 0, 100)
...
TASK.Description = function(ply)
    ...
    local description = "Kill a player from outside "
    if unit == 1 then
...
if CLIENT then
    net.Receive("TTT_Taskmaster_KillFaraway_Assigned", function()
        ...
        hook.Add("TTTPlayerAliveClientThink", "Taskmaster_KillFaraway_TTTPlayerAliveClientThink_" .. sid64, function(client, ply)
            ...
            if ply == client and client:IsActiveTaskmaster() then
                ...
                if ply.TaskmasterRadiusNextPart < CurTime() then
                    for _ = 1, 48 do
                        ...
                        ply.TaskmasterRadiusDir = ply.TaskmasterRadiusDir + math.pi / 24
                        local radius = taskmaster_killfaraway_range:GetInt() * UNITS_PER_METER
