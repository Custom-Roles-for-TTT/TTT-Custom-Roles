...
local function StartRegen(ply)
    ...
    timer.Create("LootGoblinRegen_" .. ply:EnhancedSteamID64(), rate, 0, function()
...
local function StopRegen(ply)
    timer.Remove("LootGoblinRegen_" .. ply:EnhancedSteamID64())
    timer.Remove("LootGoblinRegenDelay_" .. ply:EnhancedSteamID64())
end

local function HandleRegen(ply, delay_override)
    local delay = delay_override or lootgoblin_regen_delay:GetInt()
    if delay > 0 then
        timer.Create("LootGoblinRegenDelay_" .. ply:EnhancedSteamID64(), delay, 0, function()
            StartRegen(ply)
        end)
    else
        StartRegen(ply)
...
hook.Add("TTTBeginRound", "LootGoblin_Radar_TTTBeginRound", function()
    ...
    timer.Create("LootGoblinRadarDelay", 1, 0, function()
        for _, v in ipairs(GetAllPlayers()) do
            if v:IsActiveLootGoblin() then
                local locations = goblins[v:EnhancedSteamID64()]
                ...
                goblins[v:EnhancedSteamID64()] = locations
            end
        end
    end)
end)
