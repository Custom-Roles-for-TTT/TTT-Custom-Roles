...
ROLE_IS_TARGETID_OVERRIDDEN[ROLE_CLOWN] = function(ply, target)
    ...
    if IsClownActive(ply) and target:ShouldActLikeJester() then
        local icon_overridden, ring_overridden, text_overridden = target:IsTargetIDOverridden(ply)
        ------ icon
        return not icon_overridden,
        ------- ring
                not ring_overridden,
        ------- text
                not text_overridden
...
ROLE_IS_SCOREBOARD_INFO_OVERRIDDEN[ROLE_CLOWN] = function(ply, target)
    ...
    -- We only care about whether these are overridden if the target is a tester
    if IsClownActive(ply) and target:ShouldActLikeJester() then
        local _, role_overridden = target:IsScoreboardInfoOverridden(ply)
        ------ name,  role
        return false, not role_overridden
    end
end
...
hook.Add("TTTTutorialRoleText", "Clown_TTTTutorialRoleText", function(role, titleLabel)
    if role == ROLE_CLOWN then
        ...
        html = html .. "<span style='display: block; margin-top: 10px;'>When a team would normally win, the " .. ROLE_STRINGS[ROLE_CLOWN] .. " <span style='color: rgb(" .. roleColor.r .. ", " .. roleColor.g .. ", " .. roleColor.b .. ")'>activates</span> which converting them to be <span style='color: rgb(" .. indepColor.r .. ", " .. indepColor.g .. ", " .. indepColor.b .. ")'>independent</span> and allowing them to <span style='color: rgb(" .. traitorColor.r .. ", " .. traitorColor.g .. ", " .. traitorColor.b .. ")'>go on a rampage</span> and win by surprise.</span>"
