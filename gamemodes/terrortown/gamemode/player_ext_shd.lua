...
function plymeta:ShouldRevealBeggar(tgt)
    ...
    if tgt:IsTraitor() then
        ...
        sameTeam = self:IsTraitorTeam() and beggarMode == ANNOUNCE_REVEAL_TRAITORS
    elseif tgt:IsInnocent() then
        beggarMode = GetGlobalInt("ttt_beggar_reveal_innocent", ANNOUNCE_REVEAL_TRAITORS)
        sameTeam = self:IsInnocentTeam() and beggarMode == ANNOUNCE_REVEAL_INNOCENTS
    end

    -- Check the setting value and the player's team to see we if should reveal this beggar
    return beggarMode == ANNOUNCE_REVEAL_ALL or sameTeam
end
