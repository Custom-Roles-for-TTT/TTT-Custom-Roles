...
function ScoreEvent(e, scores)
    ...
    elseif e.id == EVENT_BODYFOUND then
        ...
        if scores[sid] == nil or scores[sid].was_traitor or scores[sid].was_hypnotist or scores[sid].was_impersonator then return end
...
function ScoreEventLog(events, scores, innocents, traitors, detectives, jesters, swappers, glitches, phantoms, hypnotists, revengers, drunks, clowns, deputies, impersonators, beggars, oldmen, mercenaries)
    for k, _ in pairs(scores) do
        scores[k] = ScoreInit()

        scores[k].was_innocent = table.HasValue(innocents, k)
        scores[k].was_traitor = table.HasValue(traitors, k)
        scores[k].was_detective = table.HasValue(detectives, k)
        scores[k].was_jester = table.HasValue(jesters, k)
        scores[k].was_swapper = table.HasValue(swappers, k)
        scores[k].was_glitch = table.HasValue(glitches, k)
        scores[k].was_phantom = table.HasValue(phantoms, k)
        scores[k].was_hypnotist = table.HasValue(hypnotists, k)
        scores[k].was_revenger = table.HasValue(revengers, k)
        scores[k].was_drunk = table.HasValue(drunks, k)
        scores[k].was_clown = table.HasValue(clowns, k)
        scores[k].was_deputy = table.HasValue(deputies, k)
        scores[k].was_impersonator = table.HasValue(impersonators, k)
        scores[k].was_beggar = table.HasValue(beggars, k)
        scores[k].was_old_man = table.HasValue(oldmen, k)
        scores[k].was_mercenary = table.HasValue(mercenaries, k)
    end
...
function ScoreTeamBonus(scores, wintype)
    local alive = { traitors = 0, innos = 0, jesters = 0 }
    local dead = { traitors = 0, innos = 0, jesters = 0 }

    for _, sc in pairs(scores) do
        local state = (sc.deaths == 0) and alive or dead
        if sc.was_traitor or sc.was_hypnotist or sc.was_impersonator then
