...
local function DoParasiteRespawnWithoutBody(parsite)
    parsite:PrintMessage(HUD_PRINTCENTER, "You have drained your host of energy and created a new body.")
    -- Introduce a slight delay to prevent player getting stuck as a spectator
    timer.Create(parsite:Nick() .. "ParasiteRespawn", 0.1, 1, function()
        DoRespawn(parsite)
        local health = GetConVar("ttt_parasite_respawn_health"):GetInt()
        parsite:SetHealth(health)
    end)
end
            ...
            if victim:IsParasite() and not victim:Alive() then
                ...
                elseif respawnMode == PARASITE_RESPAWN_BODY then
                    if IsValid(parasiteBody) then
                        ...
                        DoParasiteRespawnWithoutBody(victim)
                    end
