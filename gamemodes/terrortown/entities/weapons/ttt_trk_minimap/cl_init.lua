...
local tracker_minimap_lock_north     = CreateClientConVar("ttt_tracker_minimap_lock_north",     "0", true, false, "Whether the minimap is locked north, or rotates with the player")
local tracker_minimap_show_cardinals = CreateClientConVar("ttt_tracker_minimap_show_cardinals", "2", true, false, "Cardinal direction labels to show: none (0), North only (1), all (2)")

local tracker_minimap_range_mult     = GetConVar("ttt_tracker_minimap_range_multiplier")
local tracker_minimap_show_colours   = GetConVar("ttt_tracker_minimap_show_colours")
...
Player icon colour will match the player's footprint colour and show the direction each player is facing.]])
...
local function DrawArrow(cx, cy, w, h, angleDeg, colour)
    ...
    surface.DrawTexturedRectRotated(cx, cy, w, h, -angleDeg)
end
...
hook.Add("HUDPaint", "Tracker_Minimap_HUDPaint", function()
    ...
    local fontSize      = math.max(6, math.Round(BASE_FONT_SIZE * scale))
    local cardinalSize  = math.max(6, math.Round(BASE_CARDINAL_SIZE * scale))
    ...
    if allowEnlarge and scoreboard and IsValid(sboard_panel) and sboard_panel:IsVisible() then
        ...
        fontSize      = math.max(6, math.Round(BASE_FONT_SIZE * dynamicScale))
        cardinalSize  = math.max(6, math.Round(BASE_CARDINAL_SIZE * dynamicScale))
    end
    ...
    surface.SetDrawColor(MINIMAP_RING_COLOUR.r, MINIMAP_RING_COLOUR.g, MINIMAP_RING_COLOUR.b, MINIMAP_RING_COLOUR.a)
    surface.DrawCircle(cx, cy, radius, MINIMAP_RING_COLOUR.r, MINIMAP_RING_COLOUR.g, MINIMAP_RING_COLOUR.b, MINIMAP_RING_COLOUR.a)

    -- NESW letters
    if cardinalsMode ~= "none" then
        local function CardinalScreenAngle(worldYawDeg)
            if lockNorth then
                return math.rad(-(worldYawDeg - 90) - 90)
            else
                local delta = worldYawDeg - myYaw
                return math.rad(-delta - 90)
            end
        end
        ...
        for _, c in ipairs(cardinals) do
            if c.label == "N" then
                surface.SetFont(biggerCardinalFont)
            else
                surface.SetFont(cardinalFont)
            end

            local a  = CardinalScreenAngle(c.yaw)
            local lx = cx + math.cos(a) * cardinalOffset
            local ly = cy + math.sin(a) * cardinalOffset
            local tw, th = surface.GetTextSize(c.label)
            surface.SetTextColor(c.colour.r, c.colour.g, c.colour.b, c.colour.a)
            surface.SetTextPos(lx - tw / 2, ly - th / 2)
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        local rotAngle = lockNorth and 0 or math.rad(myYaw - 90)
        ...
        if showNames and not outsideRange then
            ...
            surface.SetTextPos(blipX - tw / 2, labelY)
