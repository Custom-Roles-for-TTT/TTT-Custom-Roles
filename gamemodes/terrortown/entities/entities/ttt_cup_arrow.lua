		...
		if ent:IsNPC() or ent:IsPlayer() then
			...
			if ent:IsPlayer() and ent:IsActive() then
				if ent == self.Owner then
					self.Owner:PrintMessage(HUD_PRINTCENTER, "You cannot make yourself fall in love with someone.")
				else
					local target1 = self.Owner:GetNWString("TTTCupidTarget1", "")
					if target1 == "" then
						ent:SetNWString("TTTCupidShooter", self.Owner:SteamID64())
						self.Owner:SetNWString("TTTCupidTarget1", ent:SteamID64())
						self.Owner:PrintMessage(HUD_PRINTCENTER, ent:Nick() .. " has been hit with your first arrow.")
						ent:PrintMessage(HUD_PRINTCENTER, "You have been hit by cupids arrow!")
					elseif self.Owner:GetNWString("TTTCupidTarget2", "") == "" then
						if ent:SteamID64() == target1 then
							self.Owner:PrintMessage(HUD_PRINTCENTER, "You cannot make someone fall in love with themselves.")
						else
							...
							self.Owner:SetNWString("TTTCupidTarget2", ent:SteamID64())
							self.Owner:PrintMessage(HUD_PRINTCENTER, ent:Nick() .. " has fallen in love with " .. ent2:Nick() .. ".")
